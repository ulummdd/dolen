<?php
  ...
  if(isset($_POST['submit'])){
    ...
    $result = mysqli_query($connect, "UPDATE tiket SET harga_tiket='$harga_tiket', kursi_tersedia='$kursi_tersedia' WHERE id_tiket='$_GET[id]'");
    if($result){
  ...
  $id = $_GET['id'];
  $show = mysqli_query($connect, "SELECT * FROM tiket WHERE id_tiket=$id");
  ...
  <body>
    ...
    <div class="row no-gutters">
        ...
        <div class="col-md-10 p-5 pt-2">
            ...
            ?>
            <div class="mb-3">
              <label for="nama_wisata" class="form-label">Tujuan wisata</label>
              <input type="text" name="nama_wisata" class="form-control" id="nama_wisata" value="<?php echo $data['nama_wisata']; ?>" disabled>
            </div>
            <div class="form-floating mb-3">
              <label for="waktu_keberangkatan">Waktu keberangkatan</label>
              <input type="datetime-local" class="form-control" name="waktu_keberangkatan" id="waktu_keberangkatan" value="<?php echo $data['waktu_berangkat']; ?>" disabled>
            </div>
            <div class="form-floating mb-3">
              <label for="harga_tiket">Harga tiket</label>
              <input type="text" class="form-control" name="harga_tiket" id="harga_tiket" value="<?php echo $data['harga_tiket']; ?>" required>
            </div>
            <div class="form-floating mb-3">
              <label for="kursi_tersedia">Kursi tersedia</label>
              <input type="text" class="form-control" name="kursi_tersedia" id="kursi_tersedia" value="<?php echo $data['kursi_tersedia']; ?>" required>
            </div>
            <div class="form-floating mb-3">
              <input type="submit" name="submit" value="Edit tiket" class="btn btn-primary mb-3">
            </div>
            <?php
              }
            ?>
